...
def test_ring_vrf_bandersnatch_root():
    ...
    root = ring_vrf.bandersnatch_root(ring_public_keys)
    assert isinstance(root, bytes)
    assert len(root) > 0
    
    # Test same ring produces same root
    root2 = ring_vrf.bandersnatch_root(ring_public_keys)
    assert root == root2
    ...
    different_root = ring_vrf.bandersnatch_root(different_ring)
    assert root != different_root
    
    # Test empty ring raises error
    with pytest.raises(ValueError):
        ring_vrf.bandersnatch_root([])
...
def test_ring_vrf_bandersnatch_root_known_values():
    ...
    ring_vrf = RingVRF()
    result = ring_vrf.bandersnatch_root(public_keys)
