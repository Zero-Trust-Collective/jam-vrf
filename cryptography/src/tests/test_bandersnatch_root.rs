...
fn test_ring_vrf_bandersnatch_root_known_values() {
    ...
    // Convert to Vec<&[u8]>
    let public_keys: Vec<&[u8]> = public_key_bytes.iter().map(|v| v.as_slice()).collect();
    ...
    Python::with_gil(|py| {
        let root = RingVRF::bandersnatch_root(public_keys).unwrap();
        assert_eq!(root.as_ref(py).as_bytes(), expected_ring.as_slice());
    });
}
